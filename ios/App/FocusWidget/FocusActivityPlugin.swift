...
public class FocusActivityPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    @objc func start(_ call: CAPPluginCall) {
        NSLog("🧋LA start() called")
        guard ActivityAuthorizationInfo().areActivitiesEnabled else {
            NSLog("🧋LA aborted — Live Activities are DISABLED in Settings")
        ...
        guard end > Date() else {
            NSLog("🧋LA aborted — computed end date is in the past")
            call.resolve(["started": false, "reason": "past"]); return
        }
        ...
        Task {
            ...
            do {
                ...
                NSLog("🧋LA started OK — activity id \(activity.id)")
                call.resolve(["started": true])
            } catch {
                NSLog("🧋LA request FAILED — \(error.localizedDescription)")
